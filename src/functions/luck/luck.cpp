...
void luck::process(std::string message, const msg_meta &conf) {
    ...
    std::string reply = "[CQ:reply,id=" + std::to_string(conf.message_id) + "]你今天的人品值是" + std::to_string(luck_val);
    
    conf.p->cq_send(reply, conf);
    
    conf.p->setlog(LOG::INFO, "luck_module: user " + std::to_string(conf.user_id) + " got " + std::to_string(luck_val));
}
